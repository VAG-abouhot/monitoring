...
from .helpers import MonitoringException, CustomJSONEncoder, rotate, urlify
...
SSL_CERTIFICATE_DOMAIN = 'validandgo.net'
...
class Transport(object):
    ...
    def _app_req(self, host, path, meth, timeout, params, data, headers):
        ...
        if res.status_code // 100 == 2 and content is not None:
            ...
            raise MonitoringException(message)
        else:
            message = '%s Server Error: %s' % (res.status_code, res.content)
    ...
    def req(self, is_search, path, meth, params, data, request_options=None):
        ...
        print(' ')
        print('-- DEBUT REQ --')
        print('hosts :', self._get_hosts(is_search))
        print('chemin :', path)
        print('type d\'action: ', meth)
        print('timeout: ', timeout)
        print('parametres: ', params)
        print('Données envoyées: ', data)
        print('headers de la requete: ', headers)
        print('-- FIN REQ --')
        print(' ')
        return (self._get_hosts(is_search), path, meth, timeout, params, data, headers)
        exceptions = {}
        ...
        raise MonitoringException('Unreachable hosts: %s' % exceptions)
